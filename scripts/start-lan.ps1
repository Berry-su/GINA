...
$env:GINA_HOST = '0.0.0.0'
$env:GINA_ALLOW_LAN = '1'
if (!$env:GINA_API_TOKEN) {
  $env:GINA_API_TOKEN = New-RandomBase64Url 32
}
...
$env:GINA_TLS_PFX = $tls.Pfx
$env:GINA_TLS_PFX_PASSPHRASE = $tls.Passphrase
$env:GINA_LAN_CA_CERT = $tls.RootCertificate

Write-Host ''
Write-Host 'Gina secure LAN mode is enabled.'
Write-Host ''
...
Write-Host '  3. Open the matching Gina URL. The pairing token is removed from the address bar automatically.'
Write-Host ''
foreach ($address in $addresses) {
  Write-Host "Certificate: https://$address`:3721/bailongma-lan-root-ca.cer"
  Write-Host "Gina:   https://$address`:3721/#token=$($env:GINA_API_TOKEN)"
}
Write-Host ''
