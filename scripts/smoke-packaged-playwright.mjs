...
const appAsar = process.env.GINA_SMOKE_APP_ASAR
const resources = process.env.GINA_PACKAGED_RESOURCES
const userDir = process.env.GINA_USER_DIR
const expectedChromium = path.resolve(process.env.GINA_EXPECTED_CHROMIUM)
const targetPlatform = process.env.GINA_SMOKE_PLATFORM
const targetArch = process.env.GINA_SMOKE_ARCH
const expectedHostPlatform = process.env.GINA_SMOKE_HOST_PLATFORM
...
assert.equal(process.env.GINA_BUNDLED_PLAYWRIGHT, '1')
...
const childEnv = Object.fromEntries(Object.entries({
  ...
  GINA_BROWSER_PRIVATE_NETWORK: '1',
...
async function runProbe() {
  const child = spawn(exe, [probeFile], {
    ...
    env: {
      ...
      GINA_USER_DIR: userDir,
      GINA_PACKAGED_RESOURCES: resources,
      GINA_SMOKE_APP_ASAR: appAsar,
      GINA_EXPECTED_CHROMIUM: chromiumExe,
      GINA_SMOKE_PLATFORM: target.platform,
      GINA_SMOKE_ARCH: target.arch,
      GINA_SMOKE_HOST_PLATFORM: target.hostPlatform,
...
function packagedChromiumPids() {
  ...
  const result = spawnSync('powershell.exe', [
    '-NoProfile', '-NonInteractive', '-Command',
    "$expected=[IO.Path]::GetFullPath($env:GINA_EXPECTED_CHROMIUM); Get-CimInstance Win32_Process -Filter \"Name='chrome.exe'\" | Where-Object { $_.ExecutablePath -and [IO.Path]::GetFullPath($_.ExecutablePath) -ieq $expected } | Select-Object -ExpandProperty ProcessId",
  ], {
    encoding: 'utf8',
    timeout: 15_000,
    windowsHide: true,
    env: { ...process.env, GINA_EXPECTED_CHROMIUM: chromiumExe },
  })
  if (result.error) throw result.error
  if (result.status !== 0) {
    throw new Error(`failed to inspect packaged Chromium processes: ${result.stderr || `exit ${result.status}`}`)
