...
const cdpPort = Number(process.env.GINA_CDP_SPIKE_PORT || 0)
const readyFile = process.env.GINA_CDP_SPIKE_READY_FILE || ''
const persistentPartition = 'persist:gina-browser-cdp-spike'

if (!Number.isInteger(cdpPort) || cdpPort <= 0) {
  throw new Error('GINA_CDP_SPIKE_PORT must contain a positive integer')
}

if (process.env.GINA_CDP_SPIKE_USER_DATA) {
  app.setPath('userData', path.resolve(process.env.GINA_CDP_SPIKE_USER_DATA))
}
...
async function startContentServer() {
  contentServer = http.createServer(async (request, response) => {
    ...
    if (url.pathname === '/brain') {
      ...
      response.end(html('Gina Brain UI spike', 'GINA_BRAIN_UI_DO_NOT_AUTOMATE'))
      return
    }
    if (url.pathname === '/browser') {
      response.writeHead(200, { 'content-type': 'text/html; charset=utf-8' })
      response.end(html('Gina browser surface spike', 'GINA_BROWSER_SURFACE'))
      return
    }
