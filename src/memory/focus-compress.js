...
export async function compressPoppedFrame(poppedFrame, currentTopFrame, { sessionRef, emitEvent, saveStack } = {}) {
  ...
  try {
    ...
    const { getRecentConversationTimeline, getRecentActionLogs, insertMemory } = await import('../capabilities/db.js')
    ...
    // 「绝大多数情况不出现」。
    try {
      const { markConversationsAbsorbed } = await import('../capabilities/db.js')
