...
const IDLE_FLUSH_MS = 5_000      // 安静这么久就把攒着的批掉
...
// 即时锚点写入：不等识别器 LLM，同步写原始对话数据到记忆库
// 解决重启发生在识别器 flush 前导致本轮经历丢失的问题
let anchorWriter = null;

export function setAnchorWriter(writer) {
  anchorWriter = writer;
}

function writeTurnAnchor(turn) {
  if (!anchorWriter) return;
  try {
    const userMsg = (turn.userMessage || '').trim();
    const topic = userMsg.slice(0, 50) || '对话';
    const summary = (turn.assistantSummary || userMsg).slice(0, 200);
    const userId = turn.userId || 'ID:000001';
    anchorWriter({ topic, summary, userId });
  } catch (e) {
    // 锚点写入失败不传播，不影响主流程
  }
}
...
export function enqueueTurnForRecognition(turn) {
  if (!turn) return
  buffer.push(turn)
  writeTurnAnchor(turn)  // 即时锚点写入：不等识别器 LLM，先落盘原始对话数据
