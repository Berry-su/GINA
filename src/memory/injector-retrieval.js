// 注入器 · 检索/排序/选择层
//
// 从 injector.js 拆出的记忆检索逻辑：消息解析、向量优先 + FTS5 补位、重要性重排、
// 「少即是强」选择器、时间词轮廓召回、概念追加召回。
// runInjector 是编排者，本文件提供它调用的检索原语。
//
// 检索策略（v2）：向量语义检索为主力（"猫"→"宠物"跨词匹配），
// FTS5 关键词索引为补位（精确字面命中 / 无 embedding 的存量记忆）。
// embedding 未配置或超时时整体退化为纯 FTS5，行为不变。

import { searchMemories, getMemoriesByDateRange } from '../capabilities/db.js'
...
//
// 旧逻辑的病灶：searchRelevantMemories 已按相关度排好序（向量优先 → FTS5 补位 → context FTS5），
...
// 相关记忆搜索：向量优先 + FTS5 补位（v2）
// focusText 是当前消息+任务+hint，享受优先权；contextText 是对话历史，作为补充。
//
// 召回顺序：向量语义召回（主力）→ FTS5 关键词补位（精确命中 / 无 embedding 的存量）→ context FTS5
// 向量命中里的 FTS5 结果会被去重，避免重复。
//
// focusText 为空时直接返回空数组，不用 contextText 兜底
// embedding 未配置/超时/异常时整体退化为旧版 FTS5-only 路径，行为不变。
export async function searchRelevantMemories({
  ...
  // ── 第一路：向量语义召回（主力） ──
  // focusText 算 embedding，cosine 找 top-N 语义相似记忆。
  // 硬超时挡在主 LLM 调用之前；本地推理无网络方差但 CPU 慢，默认 1500ms。
  let vecHitsAll = []
  try {
    ...
    if (isEmbeddingConfigured() && focusText) {
      ...
      if (queryEmb) {
        const { searchByEmbedding } = await import('../capabilities/db.js')
        vecHitsAll = searchByEmbedding(queryEmb, focusLimit)
      }
    }
  } catch {
    // 静默：embedding 异常不影响后续 FTS5 兜底
  }

  // 向量命中里 _vecScore > 0.3 的视为有效语义匹配；低于阈值的退二线给 FTS5 让位。
  const vecGood = vecHitsAll.filter(m => m._vecScore > 0.3)
  const vecIds = new Set(vecGood.map(m => m.id))
  ...
  for (const keyword of focusKws) {
    ...
    for (const memory of hits) {
      // 跳过已在向量有效命中里的（语义已覆盖，避免重复）
      if (vecIds.has(memory.id)) continue
  ...
  // ── 第三路：context FTS5（对话历史补充） ──
  const seenAll = new Set([...vecIds, ...focusHitsCapped.map(m => m.id)])
  const contextHits = []
  ...
  const vecRanked         = rerankByImportance(vecGood)
  const focusHitsRanked   = rerankByImportance(focusHitsCapped)
  const contextHitsRanked = rerankByImportance(contextHitsCapped)

  // 最终顺序：向量语义（主力）→ FTS5 补位（精确命中 / 无 embedding 存量）→ context FTS5
  return [...vecRanked, ...focusHitsRanked, ...contextHitsRanked].slice(0, focusLimit + contextLimit)
}
