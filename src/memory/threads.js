...
export function isLikelyOneOffLeaf(body) {
  ...
  // 纯寒暄/开场白：不该续上一个任务话题，也不该开新线索。
  // 放在 SUSTAINED_RE 之前——否则「测试一下」里的「测试」会被当持续任务词拦截。
  if (/^(你好|您好|哈喽|嗨|hello|hi|hey|在吗|在么|早上好|中午好|晚上好|晚安|谢谢|多谢|收到|嗯|哦|有人吗)[!！。~～\s]*$/i.test(text)) return true
  if (/^(测试一下|测试|试试|试一下|在)$/i.test(text)) return true
