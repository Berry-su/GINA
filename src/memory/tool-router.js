...
// 接单队列触发：用户转发平台需求、让 gina 接单干活时，注入接单工具。
const ORDER_TRIGGERS = [
  '接单', '接个单', '接了一单', '来单', '新单', '订单', '队列', '交付',
  '闲鱼', '淘宝', '猪八戒', '程序员客栈', '码市', 'upwork', 'fiverr', 'freelancer',
  '客户', '需求', '报价', '客单价', '这单', '那单',
  'order', 'gig', 'freelance', 'deliverable', 'client request',
]
const ORDER_TOOLS = ['enqueue_order', 'list_orders', 'complete_order']
...
export const TOOL_GROUPS = [
  ...
  { triggers: ORDER_TRIGGERS,         tools: ORDER_TOOLS },
]
...
export function selectTools(ctx = {}) {
  ...
  // 接单队列：命中接单相关词时注入，让 gina 能登记/查看/交付接单。
  if (hits(body, ORDER_TRIGGERS)) {
    for (const t of ORDER_TOOLS) out.add(t)
  }
