...
async function findSkillFilesAsync(rootDir) {
  const files = []
...
async function readSkillAsync(filePath, sourceRoot) {
  let text = ''
  try {
...
async function loadSkillsAsync({ force = false } = {}) {
  const now = Date.now()
  if (!force && cachedAt && now - cachedAt < 15000) return cachedSkills
  ...
  const byKey = new Map()
  for (const root of roots) {
    const skillFiles = await findSkillFilesAsync(root)
    for (const file of skillFiles) {
      const skill = await readSkillAsync(file, root)
      if (!skill) continue
      const key = skill.id
      if (byKey.has(key) && skill.source === 'bundled') continue
      byKey.set(key, skill)
...
export async function refreshSkills() {
  return loadSkillsAsync({ force: true })
}
