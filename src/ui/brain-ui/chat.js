...
export function initChat({
  ...
  const CHAT_PIN_STORAGE_KEY = "gina-chat-pinned";
  ...
  function collectClipboardFiles(event) {
  ...
  async function addPastedFiles(files = []) {
    ...
    const accepted = files
      .filter(file => {
        if (!file || file.size > MAX_PASTED_IMAGE_BYTES) {
          console.warn("[paste file] ignored oversized file");
          return false;
        }
        return true;
      })
    ...
    const attachments = await Promise.all(accepted.map(async (file, offset) => {
      try {
        const dataUrl = await readFileAsDataUrl(file);
        const isImage = /^data:image\//i.test(dataUrl);
        const ext = isImage ? imageExtFromMime(file.type) : (file.name.match(/\.[^.]+$/) || ['.bin'])[0];
        return {
          id: newClientMessageId(),
          data_url: dataUrl,
          alt: isImage ? "pasted image" : file.name,
          name: file.name || `pasted-file-${pendingPastedImages.length + offset + 1}${ext}`,
        };
      } catch (error) {
        console.warn("[paste file]", error?.message || error);
        return null;
      }
    }));
    ...
    for (const att of attachments) {
      if (!att || seenDataUrls.has(att.data_url)) continue;
      seenDataUrls.add(att.data_url);
      pendingPastedImages.push(att);
    }
    renderPastedImages();
    openChat();
  }
  ...
  async function send({ channel = null, label = null, text = null } = {}) {
    ...
    try {
      ...
      if (String(channel || "").toUpperCase() === "VOICE" || channel === "语音识别") {
        window.ginaVoiceDiag?.("voice-message-post", {
      ...
      if (String(channel || "").toUpperCase() === "VOICE" || channel === "语音识别") {
        window.ginaVoiceDiag?.("voice-message-accepted", {
  ...
  msgInput.addEventListener("paste", (event) => {
    const clipboardFiles = collectClipboardFiles(event);
    if (clipboardFiles.length) {
      addPastedFiles(clipboardFiles);
      openChat();
    }
  });
  ...
  function handleDrop(e) {
    ...
    if (files.length) { addPastedFiles(files); openChat(); }
  }
  ...
  // 文件选择按钮：点击 paste-attachments 区域或通过按钮触发
  if (fileInput) {
