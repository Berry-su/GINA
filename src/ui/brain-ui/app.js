...
const hasWindowsTitleBarOverlay = window.gina?.isElectron && window.gina?.platform === "win32";
...
if (hasWindowsTitleBarOverlay) {
  ...
  window.gina.onFullScreenChange?.(setFullScreenClass);
  window.gina.isFullScreen?.().then(setFullScreenClass).catch(() => {});
}
renderBrainUiApp(document.body);

// 存储键迁移：旧键名（bailongma/jarvis）→ 新键名（gina）
...
// 执行一次性迁移
const STORAGE_MIGRATION_KEY = "gina-storage-migration-done";
if (localStorage.getItem(STORAGE_MIGRATION_KEY) !== "true") {
  migrateStorageKey("jarvis-brain-ui-theme", "gina-brain-ui-theme");
  migrateStorageKey("jarvis-brain-ui-physics", "gina-brain-ui-physics");
  migrateStorageKey("bailongma_activation_warmup_until", "gina_activation_warmup_until");
  migrateStorageKey("bailongma_ui_zoom_factor", "gina_ui_zoom_factor");
  migrateStorageKey("bailongma-memory-graph-enabled", "gina-memory-graph-enabled");
  migrateStorageKey("bailongma-sse-last-event-id", "gina-sse-last-event-id");
  migrateStorageKey("bailongma_ignored_update_version", "gina_ignored_update_version");
  migrateStorageKey("bailongma_suppress_update_notifications", "gina_suppress_update_notifications");
  migrateStorageKey("bailongma-action-log-v1", "gina-action-log-v1");
  migrateStorageKey("bailongma-heartbeat-count-v1", "gina-heartbeat-count-v1");
  migrateStorageKey("bailongma-panel-l1-collapsed", "gina-panel-l1-collapsed");
  migrateStorageKey("bailongma-panel-l2-collapsed", "gina-panel-l2-collapsed");
  migrateStorageKey("bailongma-chat-pinned", "gina-chat-pinned");
  migrateStorageKey("bailongma-voice-silence-ms", "gina-voice-silence-ms");
  migrateStorageKey("bailongma-voice-provider", "gina-voice-provider");
  migrateStorageKey("bailongma-voice-mic-device-id", "gina-voice-mic-device-id");
  migrateStorageKey("bailongma-voice-diag", "gina-voice-diag");
  migrateStorageKey("bailongma-voice-watchdog", "gina-voice-watchdog");
  migrateStorageKey("bailongma-voice-output-device-id", "gina-voice-output-device-id");
  migrateStorageKey("bailongma-voice-output-banner-dismissed", "gina-voice-output-banner-dismissed");
  migrateStorageKey("bailongma.ttsfx.v2", "gina.ttsfx.v2");
  migrateStorageKey("bailongma.ttsfx.voices", "gina.ttsfx.voices");
  migrateStorageKey("bailongma.ttsfx.unlocked", "gina.ttsfx.unlocked");
  localStorage.setItem(STORAGE_MIGRATION_KEY, "true");
}

const THEME_KEY = "gina-brain-ui-theme";
const PHYSICS_STORAGE_KEY = "gina-brain-ui-physics";
const ACTIVATION_WARMUP_KEY = "gina_activation_warmup_until";
const UI_ZOOM_STORAGE_KEY = "gina_ui_zoom_factor";
const CHAT_HISTORY_PAGE_SIZE = 60;
const DEFAULT_AGENT_NAME = "Gina";
const DEFAULT_UI_ZOOM = 1.1;
const MIN_UI_ZOOM = 0.8;
const MAX_UI_ZOOM = 1.8;
const UI_ZOOM_STEP = 0.1;
const UI_ZOOM_WHEEL_STEP = 0.05;
const MEMORY_GRAPH_STORAGE_KEY = "gina-memory-graph-enabled";
const MEMORY_GRAPH_ENABLED = localStorage.getItem(MEMORY_GRAPH_STORAGE_KEY) !== "false";
const UI_CLIENT_ID = getUiClientId();
const SSE_LAST_EVENT_KEY = "gina-sse-last-event-id";
const voiceReplyCoordinator = createVoiceReplyCoordinator(UI_CLIENT_ID);
const voiceDiagnosticLog = [];
window.ginaVoiceDiagnostics = voiceDiagnosticLog;
...
window.ginaVoiceDiag = voiceDiag;
...
const IGNORED_VERSION_KEY = "gina_ignored_update_version";
const SUPPRESS_UPDATES_KEY = "gina_suppress_update_notifications";
...
function applyTheme(theme) {
  ...
  if (hasWindowsTitleBarOverlay) {
    window.gina?.setTitleBarTheme?.(theme).catch(() => {});
  }
...
window.ginaGraphLayout = graphLayoutSnapshot;
...
const ACTION_LOG_KEY = "gina-action-log-v1";
const HEARTBEAT_COUNT_KEY = "gina-heartbeat-count-v1";
...
const browserEmbedBridge = window.gina?.isElectron
  && typeof window.gina?.browserEmbed?.update === "function"
  && typeof window.gina?.browserEmbed?.hide === "function"
  ? window.gina.browserEmbed
  : null;
...
function browserEmbedPayload() {
  ...
  const rendererZoom = Math.max(
    0.25,
    Math.min(5, Number(window.gina?.getZoomFactor?.()) || 1),
  );
  return {
    mode: "card",
    visible: true,
...
function setVoiceThinking(active) {
  ...
  window.ginaVoice?.setThinking?.(thinking);
}
...
function activateTTSAudioGraph(graph) {
  ...
  window.ginaVoice?.setTTSAnalyser?.(ttsAudioGraph?.analyser || null);
}
...
function clearTTSAudioGraph(graph) {
  ...
  window.ginaVoice?.setTTSAnalyser?.(null);
}
...
async function startTTSAudio(audioEl, revokeUrl, opts = {}) {
  ...
  if (manageMic) window.ginaVoice?.suspendForTTS?.();
  ...
  return new Promise(resolve => {
    ...
    const finish = (ok, kind, error = null) => {
      ...
      if (ttsAudioEl === audioEl) {
        ...
        if (manageMic) {
          ttsCurrentText = "";
          window.ginaVoice?.resumeAfterMedia();
        }
      }
      resolve({
        ok,
...
async function pumpSttsQueue(expectedEpoch = sttsEpoch) {
  ...
  if (!sttsMicSuspended) { sttsMicSuspended = true; window.ginaVoice?.suspendForTTS?.(); }
  try {
...
function endStreamingTTS() {
  ...
  clearTTSAudioGraph();
  if (sttsMicSuspended) { sttsMicSuspended = false; window.ginaVoice?.resumeAfterMedia(); }
...
(function initMediaModes() {
  ...
  window.ginaMedia = { handle: handleMediaCommand, showVideo, controlVideo, showImage, showCamera, showMusic, controlMusic };
  ...
  (() => {
    ...
    window.addEventListener("blur", () => {
      ...
      pttHeld = false;
      document.body.classList.remove("ptt-active");
      window.ginaVoice?.pttEnd?.({ send: false });
    });
  })();
...
(function initAIVideoMode(){
  ...
  function setActive(on){
    ...
    if(active){ try{ window.ginaMedia&&window.ginaMedia.controlVideo&&window.ginaMedia.controlVideo({action:"pause"}); }catch(e){} document.body.classList.remove("video-mode"); }
  ...
  window.ginaAIVideo={ handle:handle, open:openPanel, close:closePanel };
