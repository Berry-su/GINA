...
// 渲染节能开关：主进程在窗口最小化/失焦/隐藏时通知暂停，恢复可见时继续。
// 注意：Electron(macOS) 最小化窗口不触发 visibilitychange，必须走主进程 IPC。
let renderPaused = false;
if (window.gina && window.gina.onRenderSuspend) {
  window.gina.onRenderSuspend(() => { renderPaused = true });
  window.gina.onRenderResume(() => { renderPaused = false });
}
// 浏览器兜底（非 Electron 环境）
...
export class HotspotEarth {
  ...
  _animate() {
    ...
    if (renderPaused) return;  // 页面隐藏时跳过渲染，只保留 rAF 调度
