...
const VOICE_PROVIDER_KEY = 'gina-voice-provider';
const VOICE_MIC_DEVICE_KEY = 'gina-voice-mic-device-id';
...
export function createVoiceCore({ canvas, transcript, getChatInput, getSendMessage, getLang }) {
  ...
  // ── 窗口失焦/最小化暂停（方案A：主进程 IPC 驱动 render-suspend/resume）──
  let renderPaused = false;
  function setRenderPaused(p) {
    if (renderPaused === p) return;
    renderPaused = p;
    if (p) stopRenderLoop(); else startRenderLoop();
  }
  window.gina?.onRenderSuspend?.(() => setRenderPaused(true));
  window.gina?.onRenderResume?.(() => setRenderPaused(false));
  ...
  function drawFrame(now) {
    if (renderPaused) return; // 最小化/失焦整帧跳过；恢复由 startRenderLoop 续命
  ...
  // ─── 采集诊断（定位长语音丢字根因；localStorage 'gina-voice-diag'='0' 关闭，默认开） ───
  const DIAG_ON = localStorage.getItem('gina-voice-diag') !== '0';
  let diagTimer = null;
  ...
  // 也就修了「说到几十个字球变绿后不再出字」。localStorage 'gina-voice-watchdog'='0' 可关。
  const WATCHDOG_ON = localStorage.getItem('gina-voice-watchdog') !== '0';
