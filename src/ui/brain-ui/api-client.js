const API_TOKEN_STORAGE_KEY = "gina-api-token";
const UI_CLIENT_ID_SESSION_KEY = "gina-ui-client-id";
...
if (browserWindow?.fetch) {
  ...
  browserWindow.fetch = function authenticatedFetch(input, init = {}) {
    ...
    if (!headers.has("X-Gina-Client-ID")) {
      headers.set("X-Gina-Client-ID", getUiClientId());
    }
...
export function apiWebSocketProtocols() {
  const protocols = ["gina.v1"];
  const token = getApiToken();
  if (token) protocols.push(`gina.auth.${base64UrlEncode(token)}`);
  return protocols;
}
