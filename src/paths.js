...
//   GINA_USER_DIR       - 用户数据目录（可写，存 DB、sandbox、配置）
//   GINA_RESOURCES_DIR  - 只读资源目录（存 HTML、UI 资源）
//
...
const USER_DIR = process.env.GINA_USER_DIR
  ? path.resolve(process.env.GINA_USER_DIR)
  : REPO_ROOT

const RESOURCES_DIR = process.env.GINA_RESOURCES_DIR
  ? path.resolve(process.env.GINA_RESOURCES_DIR)
  : REPO_ROOT
...
export function rescueDataFromInstallDir() {
  ...
  if (isPathInside(installDir, paths.sandboxDir)) {
    console.warn('[paths] 警告：sandbox 目录位于安装目录内，更新时会被清空，请检查 GINA_USER_DIR 配置')
    return rescued
  }
