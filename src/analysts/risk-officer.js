...
export class RiskOfficer extends Analyst {
  ...
  _localLevel(snapshot) {
    ...
    // 流动性收紧 + 地缘：降仓/警惕（L2），不直接熔断；真正的「暂停 L3」留给暴跌/去杠杆破位/跌停潮
    if (macro.liquidity === 'tight' && macro.geopoliticalRisk === 'high') l = Math.max(l, 2)
    else if (macro.liquidity === 'tight') l = Math.max(l, 1)
