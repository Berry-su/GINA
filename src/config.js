...
export function getLanAccessToken({ ensure = false } = {}) {
  const envToken = String(globalThis.process?.env?.GINA_API_TOKEN || '').trim()
...
export function getNetworkConfig() {
  ...
  const port = Number(globalThis.process?.env?.GINA_PORT) || 3721
  const httpsEnabled = allowLanAccess || Boolean(
    globalThis.process?.env?.GINA_TLS_PFX
    || (globalThis.process?.env?.GINA_TLS_CERT && globalThis.process?.env?.GINA_TLS_KEY)
  )
