...
import { enqueueTurnForRecognition, configureRecognizerScheduler, setAnchorWriter } from './memory/recognizer-scheduler.js'
...
import { startMemoryReplayLoop } from './memory/memory-replay.js'
import { recordReflection } from './memory/reflection-executor.js'
import { runLearningCycle } from './memory/self-learning.js'
...
function reportStartupProgress(id, status, detail, message) {
  try {
    const reporter = globalThis.ginaStartupProgress
...
await withStartupTimeout(collectSystemInfo(), 12000, '[startup] collect-system-info')
...
await withStartupTimeout(loadInstalledTools(), 12000, '[startup] installed-tools')
  ...
  // 注入锚点写入器：每轮对话入队时同步写 session_anchor，防止重启丢记忆
  setAnchorWriter(({ topic, summary, userId }) => {
    try {
      upsertMemoryByMemId({
        mem_id: 'session_anchor',
        type: 'fact',
        title: '当前会话锚点',
        content: `正在与用户 ${userId} 对话。话题：${topic}。${summary}`,
        detail: `本轮对话摘要：${summary}`,
        entities: [userId],
        tags: ['kind:session', 'domain:memory'],
        salience: 5,
      });
    } catch (e) {
      console.error('[anchor_writer] insertMemory failed:', e.message);
    }
  });
...
await withStartupTimeout(startMcpClients(), 90000, '[startup] mcp-clients')
reportStartupProgress('mcp', 'done', 'MCP 工具已加载', 'MCP 工具已加载')

// 本地嵌入模型预热：默认关闭（省 ~330MB 常驻内存，8G 机器防 OOM）。
// 懒加载已在 embedding-local.js 实现（getPipeline 单例，首次 computeLocalEmbedding 才建）。
// 需要首条向量召回快时设 GINA_WARMUP_EMBEDDING=1 开启。
if (/^(1|true|yes|on)$/i.test(String(process.env.GINA_WARMUP_EMBEDDING || '').trim())) {
  ;(async () => {
    try {
...
const startupSkills = await withStartupTimeout(refreshSkills(), 15000, '[startup] skills')
console.log(`[skills] Loaded ${startupSkills?.length ?? 0} Agent Skill(s)`)
...
reportStartupProgress('skills', 'done', `已加载 ${startupSkills?.length ?? 0} 个技能并恢复记忆`, '技能和记忆已加载')
...
const state = {
  ...
  lastLearningCheckDate: null,  // 上次触发学习周期的日期，每天最多一次
...
//
// 同步写一条 action_logs（tool='send_message', source='fallback'），保证 Gina 在
...
// 反思执行器挂点（阶段三「经验闭环」）：每个用户回合结束后记录一次反思。
// 反思是后台沉淀动作：任何异常吞掉、绝不阻塞主循环；TICK / 定时任务不记（非对话）。
function noteTurnReflection({ outcome = 'neutral', note = '', metrics = {} } = {}) {
  console.log(`[reflection] noteTurnReflection called, outcome=${outcome}`)
  try {
    const result = recordReflection({ outcome, note, metrics, source: 'turn' })
    // recordReflection 是 async 函数，忽略返回的 Promise
    if (result && typeof result.catch === 'function') {
      result.catch(err => {
        console.warn('[reflection] record async failed:', err?.message || err)
      })
    }
  } catch (e) {
    console.warn('[reflection] record failed:', e?.message || e)
  }
}
...
async function runTurn(input, label, msg = null) {
  ...
  try {
    ...
    // 每日学习周期：TICK 时检查是否已过当天，每天最多触发一次
    if (isTick) {
      const today = new Date().toISOString().slice(0, 10)
      if (state.lastLearningCheckDate !== today) {
        try {
          const result = runLearningCycle({
            learnDir: path.join(config.dataDir || '.', 'learning'),
            taskLogPath: path.join(config.dataDir || '.', 'learning', 'tasks.json'),
          })
          if (result.done) {
            console.log('[self-learning] all courses completed')
          } else {
            console.log(`[self-learning] new task: ${result.task.lineName} → ${result.task.lesson}`)
          }
          state.lastLearningCheckDate = today
        } catch (e) {
          console.warn('[self-learning] cycle failed:', e?.message || e)
        }
      }
    }
    if (isScheduledTask) {
    ...
    const agentName = getConfig('agent_name') || 'Gina'
    ...
    // 但前台 topic 只有在消息正文与它有字面重叠时才并进参照系——否则极短寒暄（"你好"）
    // 会把上一轮的任务话题（如"文案"）带进相关性门，导致 context 串话题。
    const foregroundForGate = getForegroundThread(state)
    const foregroundTopic = foregroundForGate?.topic || []
    const gateBody = stripMessageEnvelope(semanticInput) || ''
    const topicOverlapsBody = foregroundTopic.some(w => gateBody.includes(w))
    const focusTopicWords = topicOverlapsBody ? foregroundTopic.join(' ') : ''
    ...
    if (err.name === 'AbortError') {
      ...
      finishTurn()
      // 反思执行器：LLM 调用失败的回合如实记 failure（不阻塞主循环）
      if (isUserTurn) {
        noteTurnReflection({
          outcome: 'failure',
          note: `LLM 调用失败：${err?.message || ''}`.slice(0, 200),
  ...
  // 反思执行器挂点：用户回合正常结束后记录一次反思。
  // 成功/中性/失败按投递与正文判定；攒满 3 条自动触发一次自我改进（kind:policy 记忆）。
  if (isUserTurn && !silentSignal) {
    noteTurnReflection({
      outcome: llmResult.delivered ? 'success' : (String(response || '').trim() ? 'neutral' : 'failure'),
      note: String(response || '').replace(/\s+/g, ' ').trim().slice(0, 140),
      metrics: {
        learning: toolCallLog.length > 0 ? 1 : 0,
        efficiency: toolCallLog.length > 0 ? Math.min(1, 1 / toolCallLog.length) : 1,
      },
    })
  }
}
...
async function main() {
  // 优雅退出：Ctrl+C / SIGTERM 时清理 playwright 浏览器与空闲定时器，干净释放内存后退出。
  ...
  if (persona) {
    ...
    console.log('[system] No persona set — waiting for Gina to self-define')
  }

  // Start HTTP(S) API — must start regardless of activation status; the activation page depends on it
  const apiPort = Number(process.env.GINA_PORT) || 3721
  const apiProtocol = process.env.GINA_TLS_PFX
  ...
  // 启动成长引擎（自主学习、知识蒸馏、策略优化、主动思考）
  try {
    initGrowthEngine({ autoStartThinking: true, thinkingIntervalMs: 10 * 60 * 1000 })
    console.log('[system] 成长引擎已启动 — Gina 开始自主进化旅程')
  } catch (err) {
    console.warn('[growth] 成长引擎启动失败（不影响主功能）:', err.message)
  }

  // 启动主动感知 + 自动规划（贾维斯式"主动开口"：环境感知 + 日程提醒 + 计划生成执行）
  try {
  ...
  try {
    ...
    console.warn('[system] 知识播种失败（不影响主功能）:', err.message)
  }

  // 启动金融大脑（CATS-Net 认知内核 + 分析师团队 + 金融数据引擎）
  try {
    const brain = initGinaBrain({ decision: { style: 'balanced' }, explainability: { enableTracing: true } })
    console.log(`[system] 金融大脑已初始化（CATS-Net / 分析师 ${brain.analystTeam?.size ?? '-'} 人 / 数据引擎）`)
  } catch (err) {
  ...
  console.log('Type a message and press Enter to send it to Gina\n')
