...
import { handleConsciousnessEvent, STATES } from '../memory/consciousness-state.js'
...
export function createConsciousnessLoop({
  ...
  async function onTick() {
    ...
    try {
      ...
      if (hasMessages()) {
        ...
        // 意识状态：空闲心跳 → 可能触发反思
        try {
          const cs = handleConsciousnessEvent('idle_timeout', { tick: 'L2' })
          // 如果处于进化状态，触发主动反思
          if (cs?.current === STATES.EVOLUTIONARY) {
            try { handleConsciousnessEvent('deep_reflection', {}) } catch {}
          }
        } catch {}
      ...
      if (err?.name === 'WatchdogTimeoutError') {
        ...
        // 意识状态：错误爆发 → 反思
        try { handleConsciousnessEvent('error_burst', { error: err?.message }) } catch {}
      }
    } finally {
      processing = false
  ...
  async function startConsciousnessLoop({ runImmediateTick = true } = {}) {
    ...
    // 意识状态：启动 → 苏醒
    try { handleConsciousnessEvent('startup', { reason: 'consciousness_loop_started' }) } catch {}
    ...
    setInterruptCallback((entry) => {
      ...
      // 意识状态：用户消息到达 → 专注
      try { handleConsciousnessEvent('user_message', { fromId: entry.fromId }) } catch {}
