...
export function buildLLMMessages({ systemPrompt, contextBlock = '', conversationWindow = [], input, msg = null, recentActions = [], actionLog = [], lastToolResult = null, taskSteps = [], batteryBlock = '', currentTopic = '', isTick = false, runtimeLane = '' }) {
  ...
  // conversationWindow 来自 getRecentConversationTimeline（ORDER BY timestamp DESC，倒序：最新在前）。
  // 反转为正序（最旧在前、当前消息在最后），确保 LLM 看到的对话因果顺序正确；
  // 同时修正 formatConversationMetadata / computeExpiredFollowupSet 里"下一条"的语义假设（正序下 i+1 才是时间更晚的）。
  const sourceRows = (Array.isArray(conversationWindow) ? conversationWindow : []).slice().reverse()
