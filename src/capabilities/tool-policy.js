...
// Shell execution family. On a scheduled/reminder turn the user personally
// created that automation (e.g. "run the health-check script"), so the exec
// family may run — the per-command sandbox/danger gates below still apply.
// A pure autonomous heartbeat does NOT get this: it only gets the explicit
// read-only quick-command gate (isAutonomousReadOnlyCommand).
...
// Read-only command heads allowed for a pure autonomous heartbeat's quick shell
// inspection. Everything else (node/git/npm/curl/python/...) stays blocked:
// heartbeat turns get observation, not execution authority. The command must be
// a single invocation with no chaining/pipes/redirection/substitution and no
// mutation verbs, so a "quick" inspection cannot smuggle a side effect.
const AUTONOMOUS_READONLY_HEADS = new Set([
  'ls', 'dir', 'gci', 'get-childitem', 'gl', 'tree',
  'pwd', 'get-location',
  'cat', 'type', 'gc', 'get-content',
  'grep', 'rg', 'ag', 'findstr', 'select-string', 'sls',
  'ps', 'df', 'du', 'free', 'vm_stat', 'uptime', 'w', 'who',
  'whoami', 'hostname', 'uname', 'date', 'get-date',
  'head', 'tail', 'wc', 'nl', 'basename', 'dirname', 'stat', 'file', 'which',
  'env', 'printenv', 'echo', 'write-output',
  'find',
])
const AUTONOMOUS_READONLY_BLOCK_RE = /(?:;|\|\||&&|[|<>`]|\$\(|\brm\b|\brmdir\b|\bdel\b|\berase\b|\bmv\b|\bcp\b|\bmkdir\b|\btouch\b|\bchmod\b|\bchown\b|\bkill\b|\bkillall\b|\bpkill\b|\btaskkill\b|\bshutdown\b|\breboot\b|\bsudo\b|\bsu\b)/i

export function isAutonomousReadOnlyCommand(command = '') {
  const text = String(command || '').trim()
  if (!text || text.length > 500) return false
  if (AUTONOMOUS_READONLY_BLOCK_RE.test(text)) return false
  const head = text.split(/\s+/)[0].replace(/^['"]/, '').replace(/\.(exe|cmd)$/i, '').toLowerCase()
  if (!AUTONOMOUS_READONLY_HEADS.has(head)) return false
  return true
}
...
export function evaluateToolPolicy(name, args = {}, context = {}) {
  ...
  if (context.autonomous && AUTONOMOUS_USER_AUTH_REQUIRED.has(name) && !context.allowHighRiskAutonomy && !startupBrowserCheck) {
    // Scheduled/reminder turns are user-driven automation: the user personally
    // created the reminder, so the exec family may run the scripts it was set
    // up for. The per-command sandbox/danger gates above still apply.
    const userDrivenScheduled = EXEC_FAMILY.has(name) && !!context.scheduledContext
    // Pure heartbeat: only explicitly read-only quick shell inspection.
    const readOnlyQuickInspection = name === 'exec_quick_command'
      && isAutonomousReadOnlyCommand(args.command || args.cmd || '')
    if (!userDrivenScheduled && !readOnlyQuickInspection) {
