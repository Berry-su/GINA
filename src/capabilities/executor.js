...
import { upsertPrefetchTask, removePrefetchTask, listPrefetchTasks, setConfig as dbSetConfig, getRecentActionLogs } from '../capabilities/db.js'
...
async function executeToolUnchecked(name, args, context = {}) {
  try {
    ...
    switch (name) {
      ...
      case 'enqueue_order':
        return execEnqueueOrder(args)
      case 'list_orders':
        return execListOrders(args)
      case 'complete_order':
        return execCompleteOrder(args)
      case 'review_work':
...
function formatOrder(order) {
  ...
  return [
    ...
    `  平台: ${order.platform}${order.client ? ' | 客户: ' + order.client : ''}`,
    `  需求: ${order.requirement}`,
    `  优先级: ${order.priority}`,
    order.deliverable ? `  交付物: ${order.deliverable}` : '',
  ].filter(Boolean).join('\n')
}

function execEnqueueOrder(args = {}) {
  const result = enqueueOrder(args)
  if (!result.ok) return `错误：${result.error}`
  const stats = getOrderQueueStats()
  const next = nextPendingOrder()
  return [
    `接单已入队：${result.order.id}`,
    `  平台: ${result.order.platform} | 优先级: ${result.order.priority}`,
    `  需求: ${result.order.requirement}`,
    `队列现状: 待办 ${stats.pending} | 进行中 ${stats.inProgress} | 已完成 ${stats.done}`,
    next && next.id === result.order.id
      ? '这是当前最高优先级的待办单，现在就开始做。'
      : `下一个待办: ${next ? next.id + '（' + next.requirement.slice(0, 40) + '）' : '无'}`,
  ].filter(Boolean).join('\n')
}

function execListOrders(args = {}) {
  const { status = '' } = args || {}
  const orders = listOrders(status)
  if (orders.length === 0) return status ? `队列里没有状态为 "${status}" 的接单。` : '接单队列为空。'
  const stats = getOrderQueueStats()
  const header = `接单队列（${status ? `状态=${status}，` : ''}共 ${orders.length} 单；待办 ${stats.pending} | 进行中 ${stats.inProgress} | 已完成 ${stats.done}）：\n\n`
  return header + orders.map(formatOrder).join('\n\n')
}

function execCompleteOrder(args = {}) {
  const id = String(args?.order_id || '')
  if (!id) return '错误：缺少 order_id'
  const result = completeOrder(id, { deliverable: args?.deliverable, summary: args?.summary })
  if (!result.ok) return `错误：${result.error}`
  const next = nextPendingOrder()
  const lines = [`接单已完成：${result.order.id}（${result.order.requirement.slice(0, 60)}）`]
  if (result.order.deliverable) lines.push(`交付物: ${result.order.deliverable}`)
  lines.push(next ? `下一个待办: ${next.id}（${next.requirement.slice(0, 60)}）—— 继续做这一单。` : '队列已清空，没有待办单了。')
  return lines.join('\n')
}
...
function execTerminalStream({
  ...
  const cleanTitle = String(title || 'Gina Terminal Stream').trim() || 'Gina Terminal Stream'
...
// ── 个人数据源工具（日历/邮件/通讯录，macOS 只读）──────────────────────────

async function execCalendarEvents(args = {}) {
  const res = await fetchCalendarEvents({ days: Number(args.days) || 7 })
  return toolJson({ ok: res.available, tool: 'calendar_events', ...res })
}

async function execRecentEmails(args = {}) {
  const res = await fetchRecentEmails({ limit: Number(args.limit) || 20, includeBody: args.include_body === true })
  return toolJson({ ok: res.available, tool: 'recent_emails', ...res })
}

async function execSearchContacts(args = {}) {
  const res = await searchContacts({ query: args.query || '', limit: Number(args.limit) || 20 })
  return toolJson({ ok: res.available, tool: 'search_contacts', ...res })
}
