...
const THREE_VENDOR_ROOT = path.join(paths.resourcesDir, 'node_modules', 'three', 'build')
const PET_ASSET_ROOT = path.join(paths.resourcesDir, 'src', 'ui', 'pet')
...
export async function handleStaticRoutes(req, res, url) {
  if (req.method === 'GET' && url.pathname === '/bailongma-lan-root-ca.cer') {
    const caPath = String(globalThis.process?.env?.GINA_LAN_CA_CERT || '').trim()
    if (!caPath) {
      res.writeHead(404)
  ...
  if (req.method === 'GET' && url.pathname.startsWith('/vendor/three/')) {
    serveAsset(req, res, THREE_VENDOR_ROOT, '/vendor/three/')
    return true
  }
  ...
  if (req.method === 'GET' && url.pathname.startsWith('/src/ui/pet/')) {
    serveAsset(req, res, PET_ASSET_ROOT, '/src/ui/pet/')
    return true
  }
