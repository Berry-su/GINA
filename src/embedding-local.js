...
// 内存保护：系统空闲内存低于该阈值时不加载本地模型（bge-large-zh q8 约 330MB），
// 直接返回 null 让上层走 FTS5 兜底，避免 8G 机器加载模型瞬间触发 macOS OOM 杀进程。
const MIN_FREE_MEM_FOR_EMBEDDING = 300 * 1024 * 1024  // 300MB
...
export async function computeLocalEmbedding(text, { model, isQuery = false } = {}) {
  ...
  // 内存保护：空闲内存不足时不加载/运行本地模型，静默走 FTS5 兜底，防止 OOM。
  if (os.freemem() < MIN_FREE_MEM_FOR_EMBEDDING) {
    return null
  }
