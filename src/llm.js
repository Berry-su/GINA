...
import { routeFor, ROUTING_MODES, DEFAULT_LOCAL_MODEL } from './subagents/model-routing.js'
import { PRIVACY_LEVELS } from './subagents/protocol.js'
...
import { insertActionLog } from './capabilities/db.js'
...
let client = null
let clientKey = null
function getClient(route = null) {
  const provider = route?.provider ?? config.provider
  const baseURL = route?.baseURL ?? config.baseURL
  const apiKey = route?.apiKey ?? config.apiKey
  const signature = `${provider}|${baseURL}|${apiKey}`
...
function normalizeTemperatureForProvider(temperature, model = config.model, provider = config.provider) {
  if (typeof temperature !== 'number') return temperature
  if (shouldOmitSamplingForProviderModel(provider, model)) return undefined
  if (provider !== ZHIPU_PROVIDER) return temperature
  return Math.max(0, Math.min(1, Number(temperature.toFixed(2))))
}

function buildChatCompletionRequestParams({ messages, toolSchemas = [], temperature, topP, maxTokens, thinking = true, model = config.model, provider = config.provider }) {
  const providerTemperature = normalizeTemperatureForProvider(temperature, model, provider)
  const requestParams = {
    model,
    messages,
    stream: true,
  ...
  if (
    ...
    && !shouldOmitSamplingForProviderModel(provider, model)
  ) {
  ...
  if (provider === 'deepseek') {
    ...
    if (thinkingEnabled) {
      ...
      // DeepSeek thinking 模式要求历史 assistant 消息必须带 reasoning_content 字段，
      // 否则多轮对话报 400 "reasoning_content must be passed back to the API"。
      // 历史落库只存了 content，未存思考内容，这里补空字符串满足字段存在性校验。
      requestParams.messages = messages.map(m =>
  ...
  } else if (!thinking && shouldSendThinkingDisabledForProviderModel(provider, model)) {
    requestParams.thinking = { type: 'disabled' }
  }
  if (maxTokens) {
  ...
  if (toolSchemas.length > 0) {
    ...
    if (provider === ZHIPU_PROVIDER) requestParams.tool_stream = true
  }
  return requestParams
}

// 单次流式调用，返回 { content, toolCalls, aborted }
async function streamOnce({ messages, toolSchemas, temperature, topP, maxTokens, thinking = true, signal, onStream, model = config.model, route = null }) {
  const provider = route?.provider ?? config.provider
  ...
  const stream = await getClient(route).chat.completions.create(requestParams, { signal: reqController.signal })
...
async function streamOnceWithModelFallback(args) {
  if (args.route || config.provider !== MIMO_PROVIDER) return await streamOnceWithRetry(args)
...
export async function callLLM({ systemPrompt, message, messages: inputMessages = null, temperature = 0.5, topP = 0.9, tools = [], maxTokens, thinking = true, signal, onToolCall, onToolExecute, onStream, onRetry, toolContext = {}, mustReply = false, silentSignal = false, localReply = false, images = null, privacy = null, routingMode = null, _streamOnceForTest = null, _executeToolForTest = null }) {
  ...
  // ── 隐私路由 ──────────────────────────────────────────────────────────
  // 按数据隐私级别决定本次调用走本地模型还是云端 LLM（配合「本地大模型 + 云端 LLM 自由切换」）。
  // private 数据默认只走本地；public 走当前云端 provider。路由结果以 route 覆盖对象的形式
  // 透传到 getClient / buildChatCompletionRequestParams，不改全局 config，避免并发串扰。
  const routingDecision = routeFor(privacy ?? PRIVACY_LEVELS.PUBLIC, {
    mode: routingMode ?? ROUTING_MODES.AUTO,
    localAvailable: true,
  })
  const route = routingDecision.target === 'local'
    ? { provider: 'custom', baseURL: DEFAULT_LOCAL_MODEL.baseURL, apiKey: DEFAULT_LOCAL_MODEL.apiKey, model: DEFAULT_LOCAL_MODEL.model }
    : null
  if (route) console.log(`[隐私路由] ${routingDecision.reason} → 本地模型 ${route.model}`)
  ...
  // Embed image data URLs into the last user message as image_url content
  // parts when the provider supports multimodal (Ollama vision models).
  // Text-only providers see the markdown image links in the message text.
  if (Array.isArray(images) && images.length > 0) {
    ...
    if (lastUserIdx >= 0) {
      ...
      for (const img of images) {
        const dataUrl = img?.dataUrl || img?.data_url || ''
      ...
      if (parts.length > 1) {
        messages[lastUserIdx] = { ...userMsg, content: parts }
      }
    }
  }
  if (shouldThrottle()) {
