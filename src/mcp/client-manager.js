...
// —— playwright 空闲自动关闭：浏览器空闲超时后自动销毁，释放 Chromium 常驻内存（~300MB）——
// 懒连接已保证启动不预开浏览器；这里补「用完释放」：长时间没再碰浏览器就自动关掉，
// 下次 LLM 再调 browser_* 工具时会经 withBuiltInPlaywrightProfile 懒连接自动重启，能力不丢。
const PLAYWRIGHT_IDLE_CLOSE_MS = 15 * 60 * 1000 // 15 分钟无使用即关闭
let lastPlaywrightUseTs = 0
let playwrightIdleTimer = null

function startPlaywrightIdleReaper() {
  if (playwrightIdleTimer) return
  playwrightIdleTimer = setInterval(() => {
    if (!lastPlaywrightUseTs) return
    if (Date.now() - lastPlaywrightUseTs < PLAYWRIGHT_IDLE_CLOSE_MS) return
    const activeIds = [BUILTIN_PLAYWRIGHT_INTERACTIVE_ID, BUILTIN_PLAYWRIGHT_READER_ID]
      .filter(id => connections.get(id)?.status === 'connected')
    if (activeIds.length === 0) return
    for (const id of activeIds) {
      const role = id === BUILTIN_PLAYWRIGHT_INTERACTIVE_ID ? 'interactive' : 'reader'
      shutdownBuiltInPlaywright({ role }).catch(() => {})
    }
    console.log('[mcp] playwright 空闲自动关闭（释放 Chromium 内存）')
  }, 60 * 1000) // 每分钟检查一次
  playwrightIdleTimer.unref?.()
}
...
export async function startMcpClients() {
  startPlaywrightIdleReaper()
  return reconcileMcpClients(getRuntimeMcpServers())
}

export async function shutdownMcpClients() {
  if (playwrightIdleTimer) {
    clearInterval(playwrightIdleTimer)
    playwrightIdleTimer = null
  }
  shuttingDown = true
