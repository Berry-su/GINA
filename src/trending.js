...
export async function collectTrending(countryCode = null) {
  ...
  // 采集失败降级：所有来源都空时保留旧缓存，避免热点面板被清空、每次启动都报 0 条
  if (sources.length === 0 && stored?.sources?.length) {
    const cachedTotal = stored.sources.reduce((s, x) => s + x.items.length, 0)
    console.log('[trending] 采集失败（0 来源），保留旧缓存（' + cachedTotal + ' 条热点）')
    _cached = stored
    return stored
  }
