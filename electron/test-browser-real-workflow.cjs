...
process.env.GINA_USER_DIR = path.join(testRoot, 'user')
process.env.GINA_RESOURCES_DIR = projectRoot
fs.mkdirSync(process.env.GINA_USER_DIR, { recursive: true })
app.setPath('userData', process.env.GINA_USER_DIR)
app.commandLine.appendSwitch('remote-debugging-address', '127.0.0.1')
app.commandLine.appendSwitch('remote-debugging-port', '0')

function waitForActivePort(timeoutMs = 8_000) {
  const activePortFile = path.join(process.env.GINA_USER_DIR, 'DevToolsActivePort')
