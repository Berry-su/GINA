...
function normalizeCapture(capture, { label = 'capture' } = {}) {
  ...
  let pageSnapshots = []
  let source
  if (capture?.kind === 'gina-network-audit' && Array.isArray(capture.events)) {
    ...
    throw new TypeError(`${label} is not a supported Gina audit JSON or HAR file`)
  }
...
function detectRiskSignals(chromeSummary, ginaSummary) {
  const signals = []
  const ginaUa = ginaSummary.fingerprintHeaders.userAgent.join(' ')
  const ginaBrands = ginaSummary.fingerprintHeaders.secChUa.join(' ')
  const chromeWebdriverValues = chromeSummary.pageEnvironment
    .map(snapshot => snapshot.navigator?.webdriver)
    .filter(value => value !== undefined && value !== null)
  const ginaWebdriverValues = ginaSummary.pageEnvironment
    .map(snapshot => snapshot.navigator?.webdriver)
    .filter(value => value !== undefined && value !== null)

  if (/electron/i.test(ginaUa) || /electron/i.test(ginaBrands)) {
    signals.push({ level: '极高', fact: true, signal: 'UA 或 Client Hints 直接包含 Electron 品牌。' })
  }
  if (
    ginaWebdriverValues.includes(true)
  ...
  } else if (ginaWebdriverValues.includes(true) && chromeWebdriverValues.includes(true)) {
    signals.push({
      level: '低',
      fact: true,
      signal: '两组页面环境快照中的 navigator.webdriver 都为 true；该字段在本次采集中不能区分两组，且应检查 CDP 启动参数造成的污染。',
    })
  } else if (ginaWebdriverValues.includes(true) && chromeWebdriverValues.length === 0) {
    signals.push({
      level: '极高',
      fact: true,
  ...
  if (
    ...
    && ginaSummary.fingerprintHeaders.userAgent.length
    && JSON.stringify(chromeSummary.fingerprintHeaders.userAgent) !== JSON.stringify(ginaSummary.fingerprintHeaders.userAgent)
  ) {
    signals.push({ level: '高', fact: true, signal: '两组 User-Agent 不一致，可被低成本服务端规则直接分组。' })
  }
  if (
    chromeSummary.fingerprintHeaders.secChUa.length
    && ginaSummary.fingerprintHeaders.secChUa.length
    && JSON.stringify(chromeSummary.fingerprintHeaders.secChUa) !== JSON.stringify(ginaSummary.fingerprintHeaders.secChUa)
  ) {
    signals.push({ level: '高', fact: true, signal: '两组 sec-ch-ua 品牌/版本不一致。' })
  }
  const chromeHeaders = chromeSummary.requestHeaderNames
  const bailongmaHeaders = ginaSummary.requestHeaderNames
  ...
  if (JSON.stringify(chromeSummary.protocols) !== JSON.stringify(ginaSummary.protocols)) {
...
function compareCaptures(chromeCapture, bailongmaCapture) {
  ...
  const ginaSummary = summarizeCapture(bailongma)
  const endpointsOnlyInChrome = difference(Object.keys(chromeSummary.endpoints), Object.keys(ginaSummary.endpoints))
  const endpointsOnlyInBailongma = difference(Object.keys(ginaSummary.endpoints), Object.keys(chromeSummary.endpoints))
  ...
  return {
    ...
    differences: {
      ...
      requestHeadersOnlyInChrome: difference(chromeSummary.requestHeaderNames, ginaSummary.requestHeaderNames),
      requestHeadersOnlyInBailongma: difference(ginaSummary.requestHeaderNames, chromeSummary.requestHeaderNames),
      queryParameterNamesOnlyInChrome: difference(chromeSummary.queryParameterNames, ginaSummary.queryParameterNames),
      queryParameterNamesOnlyInBailongma: difference(ginaSummary.queryParameterNames, chromeSummary.queryParameterNames),
      diagnosticHeaders: { chrome: chromeSummary.diagnosticHeaders, bailongma: ginaSummary.diagnosticHeaders },
      fingerprintHeaders: {
        chrome: chromeSummary.fingerprintHeaders,
        bailongma: ginaSummary.fingerprintHeaders,
      },
      protocols: { chrome: chromeSummary.protocols, bailongma: ginaSummary.protocols },
      transport: { chrome: chromeSummary.transport, bailongma: ginaSummary.transport },
      cookieState: { chrome: chromeSummary.cookies, bailongma: ginaSummary.cookies },
      hasUserGesture: { chrome: chromeSummary.hasUserGesture, bailongma: ginaSummary.hasUserGesture },
      timeline: { chrome: chromeSummary.timeline, bailongma: ginaSummary.timeline },
      requestHeaderOrderSignatures: {
        chrome: chromeSummary.requestHeaderOrderSignatures,
        bailongma: ginaSummary.requestHeaderOrderSignatures,
        caveat: 'HAR/CDP 均不保证这是最终 HTTP/2、HTTP/3 或线上线序；只能比较采集 API 暴露的顺序。',
      },
    },
    riskSignals: detectRiskSignals(chromeSummary, ginaSummary),
    limitations: [
