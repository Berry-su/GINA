...
const stage = process.env.GINA_PROFILE_TEST_STAGE
if (!stage) {
  ...
  const run = nextStage => spawnSync(electronPath, [__filename], {
    env: {
      ...process.env,
      GINA_PROFILE_TEST_STAGE: nextStage,
      GINA_PROFILE_TEST_USER_DATA: userData,
    },
    encoding: 'utf8',
  })
  ...
  app.setPath('userData', process.env.GINA_PROFILE_TEST_USER_DATA)
