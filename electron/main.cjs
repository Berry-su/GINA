...
// The ESM backend is requireed into this Electron main process. Expose the
// main-process-only permission API without requiring ESM modules to require the
// special Electron built-in module themselves.
globalThis.ginaSystemPreferences = systemPreferences
...
function resolvePortableRoot() {
  ...
  const requestedRoot = process.env.GINA_PORTABLE_DIR?.trim()
...
if (PORTABLE_USER_DIR) {
  ...
  process.env.GINA_USER_DIR ||= PORTABLE_USER_DIR
}
...
function isTlsBackend() {
  return Boolean(
    process.env.GINA_TLS_PFX
    || (process.env.GINA_TLS_CERT && process.env.GINA_TLS_KEY)
  )
}
...
const STARTUP_STEPS = [
  ...
  { id: 'core', label: '启动本地核心', detail: '加载 Gina runtime' },
...
global.ginaStartupProgress = emitStartupProgress
...
// 持久化日志：把 console.* 镜像到 USER_DIR/logs/gina.log，
// 安装版没有 stdout 的情况下，卡死/崩溃后还能 tail 这个文件复盘。
// 简易 rotate：> 5MB 时把当前文件改名 .old（覆盖上一份 .old），下次写入重开。
const LOG_DIR = path.join(USER_DIR, 'logs')
const LOG_FILE = path.join(LOG_DIR, 'gina.log')
const LOG_FILE_OLD = path.join(LOG_DIR, 'gina.old.log')
...
// Hijack 一次就够；后端在同一进程，console.* 引用的是同一个 console 对象。
...
console.log(`[main] Gina ${app.getVersion()} starting, logs → ${LOG_FILE}`)
...
let mainWindow = null
const TEMPORARY_BROWSER_PARTITION = `gina-browser-temporary-${process.pid}`
...
globalThis.ginaRequestSafeStorageAccessSync = purpose => (
...
function handleNetworkDiagnosticShortcut(input) {
  ...
  if (!NETWORK_DIAGNOSTICS_ENABLED) {
    console.warn('[network-diagnostics] disabled; set GINA_NETWORK_DIAGNOSTICS=1 before launch')
    return true
  }
...
//   const { assertWebUrlAllowed } = require(path.join(CODE_ROOT, 'src', 'capabilities', 'tools', 'web', 'url-policy.js'));
//   const runtimeConfig = require(path.join(CODE_ROOT, 'src', 'config.js'));
...
global.getGinaWindowLayoutSnapshot = getGinaWindowLayoutSnapshot
globalThis.ginaBrowserEmbedBridge = Object.freeze({
...
function validatePackagedNativeModules() {
  ...
  if (issues.length) {
    throw new Error(`Packaged native module integrity check failed:\n${issues.join('\n')}\nPlease close Gina and reinstall it with the official installer.`)
  }
}

async function bootstrapBackend(port) {
  process.env.GINA_USER_DIR ||= USER_DIR
  process.env.GINA_RESOURCES_DIR ||= RESOURCE_ROOT
  process.env.GINA_PORT = String(port)
  validatePackagedNativeModules()
  await import(BACKEND_ENTRY)
}

// 退出原因诊断：捕获所有退出路径与信号
process.on('exit', (code) => console.log('[main][diagnostic] process exit code=' + code))
process.on('SIGTERM', () => console.log('[main][diagnostic] SIGTERM received'))
process.on('SIGINT', () => console.log('[main][diagnostic] SIGINT received'))
process.on('SIGHUP', () => console.log('[main][diagnostic] SIGHUP received'))
process.on('uncaughtException', (err) => console.error('[main][diagnostic] uncaughtException:', err?.stack || err))
process.on('unhandledRejection', (err) => console.error('[main][diagnostic] unhandledRejection:', err))
...
function isLanAccessConfigured() {
  if (/^(1|true|yes|on)$/i.test(String(process.env.GINA_ALLOW_LAN || '').trim())) return true
  const userDir = process.env.GINA_USER_DIR?.trim() || USER_DIR
  try {
...
async function loadMainApp(targetWindow = mainWindow) {
  ...
  try {
    // 新 UI（apps/web, React+Vite）：设置 GINA_UI_URL 时优先加载（dev: http://localhost:5173）
    // 未设置时保持旧行为（后端托管的旧 brain-ui），保证可回退
    const uiOverride = process.env.GINA_UI_URL
    await targetWindow.loadURL(uiOverride || backendUrl(backendPort, '/'))
    emitStartupProgress({ id: 'interface', status: 'done', completed: true, message: '启动完成' })
  } catch (e) {
    if (!targetWindow.isDestroyed()) {
      throw e
    }
    // Window was destroyed during load, this is OK
    console.warn('[window] Target window destroyed during loadMainApp')
  }
}
...
async function createWindow({ loadStartup = true, show = true, assignAsMain = true, bounds = null } = {}) {
  ...
  // 3D 地球渲染节能：窗口最小化/失焦/隐藏时通知渲染进程暂停动画。
  // Electron 的 document.hidden/visibilitychange 在 macOS 最小化窗口时不触发，
  // 且 backgroundThrottling:false 下动画不被节流，必须由主进程窗口事件驱动。
  const suspendRender = () => { if (!window.isDestroyed()) window.webContents.send('render-suspend') }
  const resumeRender = () => { if (!window.isDestroyed()) window.webContents.send('render-resume') }
  window.on('minimize', suspendRender)
  window.on('restore', resumeRender)
  window.on('hide', suspendRender)
  window.on('show', resumeRender)
  window.on('blur', suspendRender)
  window.on('focus', resumeRender)
...
async function replaceStartupWithMainApp() {
  ...
  if (hasStartupWindow) {
    try {
      bounds = startupWindow.getBounds()
      wasMaximized = startupWindow.isMaximized()
      wasFullScreen = startupWindow.isFullScreen()
      wasVisible = startupWindow.isVisible()
    } catch (e) {
      // Window might have been destroyed during checks
      console.warn('[startup] Failed to read startup window state:', e.message)
    }
  }
  ...
  try {
    ...
    if (hasStartupWindow && !startupWindow.isDestroyed()) {
      try {
        browserEmbedHost.transferMainWindow(startupWindow, readyWindow)
      } catch (e) {
        console.warn('[startup] Failed to transfer main window:', e.message)
      }
    }
    ...
    // Safely destroy startup window
    if (hasStartupWindow && !startupWindow.isDestroyed()) {
      try {
        startupWindow.destroy()
      } catch (e) {
        console.warn('[startup] Failed to destroy startup window:', e.message)
      }
    }
  } catch (err) {
    if (readyWindow && !readyWindow.isDestroyed()) {
      try { readyWindow.destroy() } catch (e) { /* ignore */ }
    }
    throw err
  }
}
...
async function showMainWindow() {
  ...
  try {
    ...
    // Window might be in a bad state, recreate it
    console.warn('[window] Failed to show main window, recreating:', e.message)
    await createWindow({ loadStartup: !backendPort })
  }
}
...
function getGinaWindowLayoutSnapshot() {
  const { screen } = require('electron')
...
function createTerminalStreamWindow(payload = {}) {
  const { title = 'Gina Terminal Stream', stream_id = 'default' } = payload
  const cleanTitle = String(title || 'Gina Terminal Stream').slice(0, 120)
...
// ─── Gina 人型桌面宠物窗口 ───
// 透明/无边框/置顶，默认停靠屏幕右下角；整窗可拖拽移动（见 pet.html 的 -webkit-app-region: drag）。
// 仅负责渲染 3D 人型模型；对话/记忆/情感后续经后端 /message、/events 接入。
function createPetWindow() {
  if (petWindow && !petWindow.isDestroyed()) return
  const { workArea } = require('electron').screen.getPrimaryDisplay()
  const W = 320, H = 420, margin = 24
  ...
  petWindow.loadURL(backendUrl(backendPort, '/src/ui/pet/pet.html'))
...
ipcMain.handle('browser-embed:update', (event, options) => {
  const sender = requireMainWindowSender(event)
  if (!sender) return { ok: false, reason: 'no main window' }
  return browserEmbedHost.update(sender, options)
})
ipcMain.handle('browser-embed:hide', event => {
  const sender = requireMainWindowSender(event)
  if (!sender) return { ok: false, reason: 'no main window' }
  return browserEmbedHost.hide(sender)
})
ipcMain.handle('browser-embed:get-state', event => {
  const sender = requireMainWindowSender(event)
  if (!sender) return { ok: false, reason: 'no main window' }
  return browserEmbedHost.getState(sender)
})
...
app.on('before-quit', (event) => {
  ...
  const shutdowns = [
    globalThis.shutdownGinaMcpClients,
...
app.whenReady().then(async () => {
  ...
  try {
    ...
    dialog.showErrorBox('Startup failed', `Unable to start the Gina backend:\n${err.message}`)
    app.quit()
    return
  }
  ...
  try {
    ...
    console.error('[main] Failed to load Gina UI, will retry:', err?.stack || err?.message || err)
    emitStartupProgress({ id: 'interface', status: 'error', error: true, message: `进入界面失败，正在重试...` })
    // Try to recover: create a fresh window
    try {
      mainWindow = null
      await createWindow({ loadStartup: false, show: true, assignAsMain: true })
      if (mainWindow && !mainWindow.isDestroyed()) {
        await loadMainApp(mainWindow)
      }
      console.log('[main] UI recovery succeeded')
    } catch (recoveryErr) {
      console.error('[main] UI recovery failed:', recoveryErr?.stack || recoveryErr?.message || recoveryErr)
      emitStartupProgress({ id: 'interface', status: 'error', error: true, message: `进入界面失败: ${err.message}` })
      // Only quit if recovery also fails
      if (!mainWindow || mainWindow.isDestroyed()) {
        dialog.showErrorBox('Startup failed', `Unable to load the Gina interface:\n${err.message}`)
        app.quit()
        return
      }
    }
  ...
  // 桌面宠物：默认常驻（透明置顶，不抢主流程）
  createPetWindow()

  // 语音唤醒:默认关闭(8G 低配机 sherpa-onnx 常驻子进程会加重 OOM 风险)。
  // 语音对话尚未接入,关闭不影响打字对话。设 GINA_ENABLE_WAKE=1 可开启。
  const wakeEnabled = /^(1|true|yes|on)$/i.test(String(process.env.GINA_ENABLE_WAKE || '').trim())
  if (!wakeEnabled) {
    console.log('[main] 语音唤醒已禁用(省内存防 OOM;GINA_ENABLE_WAKE=1 可开启)')
  } else {
    try {
